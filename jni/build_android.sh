...
case `uname` in
   ...
   *)
   echo "Unsupported OS detected. Please use Linux or Mac OS X."
esac
#PREBUILT=$NDK/toolchains/arm-linux-androideabi-4.4.3/prebuilt/windows
#FFMPEG_PATH=ffmpeg-0.8.5/
FFMPEG_PATH=ffmpeg-master/
#copy our patched version of flvdec into the ffmpeg tree
FFMPEG_PATH=`basename $FFMPEG_PATH`
FFMPEG_VERSION=`cat $FFMPEG_PATH/RELEASE`
case $FFMPEG_VERSION in
   0.8*) 
   echo "Using 0.8 version of flvdec.c"
   cp flvdec-0.8.c $FFMPEG_PATH/libavformat/flvdec.c;;
   0.9*) 
   echo "Using 0.9 version of flvdec.c"
   cp flvdec-0.9.c $FFMPEG_PATH/libavformat/flvdec.c;;
   0.10*)
   echo "Using 0.10 version of flvdec.c"
   cp flvdec-0.10.c $FFMPEG_PATH/libavformat/flvdec.c;;
   1.0*)
   echo "Using 1.0 version of flvdec.c"
   cp flvdec-1.0.c $FFMPEG_PATH/libavformat/flvdec.c;;
   *) echo "UNKNOWN FFMPEG VERSION"
esac
...
#echo "REMOVE --disable-optimizations before publishing!"
make clean
make  -j4 install

$PREBUILT/bin/arm-linux-androideabi-ar d libavcodec/libavcodec.a inverse.o

$PREBUILT/bin/arm-linux-androideabi-ld -rpath-link=$PLATFORM/usr/lib -L$PLATFORM/usr/lib  -soname libffmpeg.so -shared -nostdlib  -z,noexecstack -Bsymbolic --whole-archive --no-undefined -o $PREFIX/libffmpeg$FFMPEGBUILD.so libavcodec/libavcodec.a libavformat/libavformat.a libavutil/libavutil.a  -lc -lm -lz -ldl -llog  --warn-once  --dynamic-linker=/system/bin/linker $PREBUILT/lib/gcc/arm-linux-androideabi/4.4.3/libgcc.a
